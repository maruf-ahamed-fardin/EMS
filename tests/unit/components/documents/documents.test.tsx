import type { DocumentItem, DocumentTypeItem } from '@/lib/validations';
import { fireEvent, render, screen, waitFor } from '@testing-library/react';
import { ApiRequestError } from '@/lib/client/api-error';
import { checkDocumentFile, documentsHref, formatBytes, titleFromFileName } from '@/lib/client/documents';
...
vi.mock('@/lib/client/api-client', () => ({ api: (...args: unknown[]) => api(...args), apiUpload: (...args: unknown[]) => apiUpload(...args) }));
const download = vi.fn();
vi.mock('@/lib/client/documents', async (original) => ({ ...(await original<typeof import('@/lib/client/documents')>()), downloadDocument: (url: string) => download(url) }));
