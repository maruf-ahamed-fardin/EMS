...
one route file per endpoint under `app/api/v1`, generated by `yarn api:routes`), one `package.json` at the repository root.
...
- **After adding or changing an endpoint** in a controller, run `yarn api:routes`; `yarn check:routes`
  (in CI) fails when a route file under `app/api/v1` is missing or stale.
