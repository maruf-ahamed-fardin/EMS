import { type AuditDetail, can, type DataResponse } from "@ems/contracts";
...
export default async function AuditEntryPage({
  ...
  if (!session || !can(session.permissions, "audit.view", "ALL"))
    return <Forbidden />;
  ...
  try {
    entry = (
      await serverApiJson<DataResponse<AuditDetail>>(
    ...
    <div className="grid gap-6">
      ...
      <section className="rounded-2xl border bg-card p-5 shadow-panel md:p-6">
        ...
        <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-sm">
          ...
          {entry.actor && (
            <Link
              href={auditHref({}, { actorUserId: entry.actor.id })}
              className="font-medium text-primary hover:underline"
            >
              ...
              {entry.changes.map((change) => (
                <TableRow key={change.field} className="align-top">
                  ...
                  {change.hidden ? (
                    <TableCell
                      colSpan={2}
                      className="pr-5 text-muted-foreground"
                    >
                      <span className="inline-flex items-center gap-1.5">
                        <Lock className="size-3.5" aria-hidden /> Private. Your
                        role can&rsquo;t see employees&rsquo; private details.
                      </span>
                    </TableCell>
                  ) : (
                    <>
                      ...
                      <TableCell className="pr-5 whitespace-pre-line">
