...
export default async function ReportsPage({
  ...
  if (!session || !can(session.permissions, "report.view"))
    return <Forbidden />;

  const raw = await searchParams;
  const key = reportKey(raw.report);
  const filters = reportParams(key, {
    ...defaultRange(key, dhakaToday()),
    ...raw,
  });
  ...
  return (
    <div className="grid gap-6">
        ...
        {REPORT_KEYS.map((k) => (
          <Link
            ...
            className={cn(
              ...
              k === key &&
                "border-primary/30 bg-accent text-accent-foreground hover:bg-accent",
            )}
          >
            {REPORT_TITLES[k]}
          </Link>
      ...
      <section
        className="rounded-2xl border bg-card shadow-panel"
      ...
      >
        <ReportFilters
          ...
          leaveTypes={(leaveTypes?.data ?? []).map((t) => ({
            value: t.id,
            label: t.name,
          }))}
        />
        {canExport && report && (
          <div className="flex flex-wrap items-center gap-2 p-4">
            <span className="mr-1 text-sm text-muted-foreground">
              Download all {report.meta.total.toLocaleString("en-US")} rows:
            </span>
            ...
            {tooMany && (
              <p className="flex w-full items-center gap-2 text-sm text-warning-text">
                <FileWarning className="size-4" aria-hidden /> Downloads stop at{" "}
                {MAX_EXPORT_ROWS.toLocaleString("en-US")} rows. Narrow the dates
                or add a filter.
              </p>
            )}
          </div>
      ...
      {problem ? (
        ...
        <StatePanel
          ...
          description="Try other dates or remove a filter."
        />
      ) : (
        report && (
          <>
            ...
            <section
              className="overflow-hidden rounded-2xl border bg-card shadow-panel"
            ...
            >
              ...
              <div className="border-t">
                <Pagination
                  meta={report.meta}
                  noun="rows"
                  hrefFor={(p) => reportsHref(key, filters, p)}
                />
              </div>
            </section>
          </>
