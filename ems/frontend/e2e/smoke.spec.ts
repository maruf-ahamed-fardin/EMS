...
async function expectAccessible(page: Page, where: string) {
  ...
  expect(serious.map((v) => `${where}: ${v.id} (${v.impact}) ${v.help} — ${v.nodes.map((n) => n.target.join(' ')).slice(0, 3).join(', ')}`)).toEqual([]);
}
...
test.describe.serial('smoke', () => {
  ...
  test('HR sees the dashboard and profile, and approves that request', async ({ page }) => {
    const errors = watchErrors(page);
    ...
    await expect(page.getByRole('heading', { level: 1, name: 'Rahim Ahmed' })).toBeVisible();
    ...
    await expect(page.getByText('Leave approved for Rahim Ahmed')).toBeVisible();
