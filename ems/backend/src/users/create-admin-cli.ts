 ...
 * Refuses when any Super Admin already exists, so it can't be used to take over a running system.
 */
async function main() {
  try {
  ...
  try {
    ...
    if (await prisma.user.count({ where: { roleId: role.id } })) throw new Error('A Super Admin already exists. Add more accounts in Users.');
