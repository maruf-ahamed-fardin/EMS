...
import { z } from 'zod';
import { USABLE_SUPER_ADMIN } from '@/lib/auth/account-protection';
...
/**
 * First deploy only: `yarn tsx lib/services/users/create-admin-cli.ts admin@example.com` (after the release step).
...
async function main() {
  ...
  if (!email.success) throw new Error('Usage: yarn tsx lib/services/users/create-admin-cli.ts admin@example.com');
