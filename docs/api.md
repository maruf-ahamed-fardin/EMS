...
| `GET /team-profile` | `team_profile.view` | The staff directory. Query: `page`, `limit`, `q` (every word must match a name, employee code or email), `departmentId`, `workLocation`. Each item: name, code, position, department, location, email, business number, headline, links, manager and joining date — the full card minus personal number and blood group. Returns `{ data, meta, mode, ambiguous }`. With `team_profile.browse`, `mode: "browse"`: a filtered page of everyone. Without it, `mode: "lookup"`: nothing until `q` is given, then only the person it names (exact employee code or email, or a name). When everyone matched shares the same full name, all of them are returned (at most 10), each with their own employee code; matches with different names return nobody with `ambiguous: true`, and filters and paging are ignored. |
| `GET /team-profile/filters` | `team_profile.browse` | Departments and work locations that have someone listed, for the filter menus |
...
| `PATCH /team-profile/me` | `team_profile.manage_own` | `{ businessPhone?, bloodGroup?, headline?, showPersonalPhone?, links? }` → the updated own card. Link kinds: `FACEBOOK`, `INSTAGRAM`, `DISCORD`, `GITHUB`, `LINKEDIN`, `WEBSITE`; send only the ones the person switched on. Links must be https, at most one per kind, and on that network's own site (`TEAM_PROFILE_LINK_HOSTS`; a website may be anywhere). Audited as `team_profile.updated`. |
| `POST /team-profile/me/photo` | `team_profile.manage_own` | Multipart, field `photo`, at most 2 MB → the updated own card. The type is read from the bytes (JPEG, PNG or WebP; SVG and anything else refused). Stored under a new server-made key; the old file is deleted after the change commits. Audited as `team_profile.photo_updated`. The web app crops to a 512 px square and re-encodes first, which drops camera metadata. |
| `DELETE /team-profile/me/photo` | `team_profile.manage_own` | Removes the photo and deletes its file → the updated own card. Audited as `team_profile.photo_removed`. |
| `GET /team-profile/:employeeId/photo` | `team_profile.view` | The photo of a listed person, inline, with `Cache-Control: private, immutable` (the `v` in `photoUrl` changes with every photo), `nosniff` and `sandbox` CSP. 404 when there is none. Cards carry `photoUrl`, never the storage key. |
| `GET /team-profile/:employeeId` | `team_profile.view` | One card: name, code, position, department, location, email, the numbers the person publishes, blood group, headline, links, manager and joining date. 404 for someone inactive or deleted. |
