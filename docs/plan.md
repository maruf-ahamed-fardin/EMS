...
Status: **decided; Phases 1–12 built and tested against PostgreSQL** (2026-09-19). Nothing is deployed yet; see `deployment.md`.
Date: 2026-09-17
...
>   **Superseded on 2026-09-20:** the Team-SeloraX app was retired and the EMS moved from `ems/` to the
>   repository root, so `selorax-ems/backend` is now simply `backend`. The repository is the EMS.
> - **D2–D7:** the recommendations are accepted as written.
> - **Local services:** Docker Desktop (Postgres and Mailpit in `docker-compose.yml`).
...
```
selorax-teamprofile/          the repository root; see D1
