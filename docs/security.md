...
| Card photos | Type read from the bytes (JPEG, PNG, WebP; never SVG), 2 MB cap, server-made keys that never leave the API (cards get a hashed-version URL), served only for listed people behind `team_profile.view` with `nosniff` and a `sandbox` CSP. The browser crops and re-encodes before upload, dropping EXIF such as GPS | `team-profile/photo.ts`, `team-profile.service.ts`, `photo-editor.tsx` | `team-profile-photo.spec.ts`, `team-profile-shape.spec.ts` |
| Card QR and NFC | QR codes and NFC tags are built in the browser from what the card already shows, so they reveal nothing more. A scanned code opens a page only when it is one of our own card links (same origin, `/team-profile/<uuid>`); anything else is displayed, never followed. The camera is allowed for this origin only (`Permissions-Policy: camera=(self)`) | `lib/qr.ts`, `lib/web-nfc.ts`, `next.config.ts` | `qr.test.ts` |
