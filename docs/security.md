...
| Team Profile exposure | The directory is visible to everyone, so its API reads its own short column list (`CARD_SELECT`) instead of the employee one; a person can hide their personal number; blood group is optional and self-declared | `team-profile.service.ts`, `permissions.md` | `team-profile-shape.spec.ts` |
